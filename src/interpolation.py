...
class Interpolator:
    """
    A base class for interpolation methods.

    Attributes:

        data (GeoDataFrame): The input data.
        params (dict): Dictionary of parameters for interpolation. Possible keys include:
            ...
            grid_spacing (float): Spacing for the grid.
            metric (str): Metric to be interpolated.
            epsg (int): EPSG code for the coordinate reference system.
        shapefile (str): Path to the shapefile for the region.
        test_data (GeoDataFrame): Test data for validation. If not provided, interpolation will be performed over the entire region.
        interpolate_to_grid (bool): If True, interpolation will be performed over a grid covering the entire region.
    """
    def __init__(self, data, params, shapefile=None, test_data=None, interpolate_to_grid=True):
        self.params = params
    ...
    def fit(self, n_jobs=-1):
        """
        Fits an interpolation model to the data.

        Returns:
            sdata: A dataframe containing smoothed values for each cell along with weights to filter outliers.
        """
...
class STBKR_Interpolator(Interpolator):
    ...
    def __init__(self, data, params, c, k, shapefile=None, test_data=None, interpolate_to_grid=True):
        super().__init__(data, params, shapefile, test_data, interpolate_to_grid=interpolate_to_grid)
        self.c = c
        self.k = k
...
class LOESS_Interpolator(Interpolator):
    ...
    def __init__(self, data, params, span, degree,shapefile=None, test_data=None, interpolate_to_grid=True):
        super().__init__(data, params, shapefile=shapefile, test_data=test_data, interpolate_to_grid=interpolate_to_grid)
        self.span = span
        self.degree = degree
...
class IDW(Interpolator):
    def __init__(self, data, params, p, shapefile=None, test_data=None, interpolate_to_grid=True):
        super().__init__(data, params, shapefile=shapefile, test_data=test_data, interpolate_to_grid=interpolate_to_grid)
        self.p = p
